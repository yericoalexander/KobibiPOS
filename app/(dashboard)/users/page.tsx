        ...
        <form onSubmit={handleAdd} className="space-y-4">
          <div>
            ...
            <input 
              ...
              className="w-full bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-xl py-2.5 px-4 text-sm text-[var(--color-text)] focus:outline-none focus:border-[var(--color-accent)] transition-all" 
            />
          </div>
          <div>
          ...
          <div>
            ...
            <select 
              ...
              className="w-full bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-xl py-2.5 px-4 text-sm text-[var(--color-text)] focus:outline-none focus:border-[var(--color-accent)] transition-all cursor-pointer"
            >
          ...
          <button 
            type="submit" 
            className="w-full bg-[var(--color-accent)] hover:opacity-90 text-white font-bold py-3 mt-2 rounded-xl flex justify-center items-center shadow-md shadow-blue-500/20 transition-all text-sm"
          >
      ...
      <div className="flex-1 space-y-4">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-[var(--color-text-muted)] mt-1 text-sm">Kelola staf yang memiliki akses ke sistem POS Anda</p>
          </div>
          <div className="relative w-full sm:max-w-xs">
            ...
            <input 
              ...
              className="w-full bg-white border border-[var(--color-border)] rounded-xl py-2.5 px-10 focus:outline-none focus:border-[var(--color-accent)] transition-all text-sm pr-10 shadow-sm"
              value={search}
        ...
        <div className="bg-white border border-[var(--color-border)] rounded-2xl overflow-hidden shadow-sm">
          {loading ? (
            <div className="p-10 text-center text-[var(--color-text-muted)] text-sm animate-pulse">Memuat team...</div>
          ) : (
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-[var(--color-surface-2)] text-[var(--color-text-muted)]">
                <tr>
                  <th className="px-6 py-4 font-semibold uppercase tracking-wider text-[10px]">Nama</th>
              ...
              <tbody className="divide-y divide-[var(--color-border)]">
                {filteredUsers.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="px-6 py-8 text-center text-[var(--color-text-muted)] italic text-xs">
                      {search ? 'Team tidak ditemukan' : 'Belum ada team'}
                    </td>
                  </tr>
                ) : (
                  filteredUsers.map(u => (
                    <tr key={u.id} className="hover:bg-[var(--color-surface-2)] transition-colors">
                      <td className="px-6 py-4 font-semibold flex items-center text-[var(--color-text)]">
                        <div className="w-8 h-8 rounded-full bg-[var(--color-accent)]/10 text-[var(--color-accent)] flex items-center justify-center mr-3 font-bold text-xs">
                      ...
                      <td className="px-6 py-4">
                        <span className="px-2.5 py-1 rounded-md text-[10px] font-bold bg-[var(--color-accent)]/10 text-[var(--color-accent)]">
                          {u.role}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <span className={`px-2.5 py-1 rounded-md text-[10px] font-bold ${u.active ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-500'}`}>
