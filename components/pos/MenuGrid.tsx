...
export default function MenuGrid({ products }: { products: any[] }) {
  ...
  return (
    <div className="overflow-y-auto h-full pr-2 pb-24 custom-scrollbar">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
        {products.map((p) => {
          ...
          return (
            <div
              ...
              className={`group relative flex flex-col justify-between p-4 bg-[var(--color-surface)] border ${currentQty > 0 ? 'border-[var(--color-accent)]' : 'border-[var(--color-border)]'} rounded-xl transition-all duration-200 ${currentQty === 0 ? 'hover:border-[var(--color-accent)]/50 active:scale-95 touch-manipulation cursor-pointer hover:bg-[var(--color-surface-2)] shadow-sm' : ''}`}
            >
              <div className="flex justify-between items-start mb-2">
                <span className="font-bold text-sm leading-snug flex-1 pr-2 line-clamp-2">
                  {p.name}
                </span>
              </div>
              
              <div className="flex items-end justify-between gap-2 mt-auto">
                <span className="text-xs font-mono text-[var(--color-accent)] font-bold bg-[var(--color-accent)]/10 px-2 py-1 rounded-md">
                ...
                {currentQty > 0 ? (
                  <div className="flex flex-col items-end gap-1.5">
                    ...
                    <div className="flex items-center bg-[var(--color-background)] rounded-lg p-1 border border-[var(--color-border)] h-8" onClick={(e) => e.stopPropagation()}>
                      <button
                        onClick={(e) => handleDecrease(p, currentQty, e)}
                        className="w-7 h-full flex items-center justify-center text-[var(--color-text-muted)] hover:text-white hover:bg-white/10 rounded-md transition-colors active:scale-95"
                      >
                        −
                      </button>
                      ...
                      <button
                        ...
                        className="w-7 h-full flex items-center justify-center text-[var(--color-accent)] hover:bg-[var(--color-accent)]/20 rounded-md transition-colors active:scale-95"
                      >
                        +
                      </button>
                    </div>
        ...
        {products.length === 0 && (
          <div className="col-span-full flex items-center justify-center text-[var(--color-text-muted)] text-sm italic py-10">
